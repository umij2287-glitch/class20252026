...
import com.demoweb.dao.MyBatisBoardDao;
...
public class BoardController {
    ...
    private final MyBatisBoardDao boardDao_1;
	...
	public BoardController(BoardDao boardDao, AuthController authController, MyBatisBoardDao boardDao_1, EtcController etcController) { // 생성자의 전달인자는 자동으로 의존 객체 주입.
		this.boardDao = boardDao; // 생성자의 전달인자는 자동으로 의존 객체 주입.
		this.authController = authController; // 생성자의 전달인자는 자동으로 의존 객체 주입.
		this.boardDao_1 = boardDao_1; // 생성자의 전달인자는 자동으로 의존 객체 주입.
...
//	}
	...
	public String boardView(
			...
			Model model) { // Model 타입 전달인자는 View 로 데이터를 전달하는 도구.
		int pageSize = 7; // 한 페이지에 보여줄 갯수.
		int pagerSize= 4; // 한번에 표시되는 페이지 번호의 갯수.
...
//		}
		...
		System.out.println("--------------->" + newBoard.getBoardNo()); // 글 쓰기 전의 boardNo
		
		boardDao.insertBoard(newBoard);
		
		System.out.println("--------------->" + newBoard.getBoardNo()); // 글 쓴 후의 boardNo
		
		if (newBoard.getAttachments() != null && newBoard.getAttachments().size() > 0) {
		// if (newBoard.getAttachments() != null || newBoard.getAttachments().size() > 0) {
		// 위에서 or 이면 attachments 가 null 인 경우 getAttachments.size() 에서 nullpointexception 에러 발생. 	
